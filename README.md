...
- **출근 / 퇴근** — 출근 버튼을 누른 시간부터만 측정, 앱 종료 시 자동 퇴근
- **외출** — 사유 입력 후 외출 처리, 해당 시간은 측정 제외
- **식사 시간 자동 제외** — 평일/주말 점심·저녁 시간 자동 제외
- **VMware 감지** — VMware 창 화면 변화 감지로 가상머신 작업도 활동으로 인정
- **치트 감지** — 키보드에 물건 올리기·매크로 자동 감지 및 측정 중단
- **일간 / 주간 / 월간 랭킹** — 기간별 누적 공부 시간 대시보드
- **주간 랭킹 자동 공지** — 매주 일요일 09:00에 일별 점수제(1위 3점·2위 2점·3위 1점) 합산 랭킹 자동 발표
- **목표 시간 설정** — 그룹별/전체 하루 목표 시간 + 달성률 진행바 표시
- **목표 미달 경고** — 오후 6시 기준 목표 50% 미달 시 트레이 알림
- **실시간 출석 현황** — 지금 누가 출근 중인지 대시보드에서 확인
- **내 통계 페이지** — 일별 활동 차트, 주간 요약, 연속 달성 스트릭, 요일별 평균 패턴, 외출 통계
- **알 부화 시스템** — 누적 공부시간에 따라 알이 자라고 동물로 부화 (SVG 일러스트)
- **비밀번호 변경** — 부원이 클라이언트 앱에서 직접 변경 가능
- **피드백 현황 조회** — 부원이 제출한 피드백의 처리 상태·관리자 코멘트를 피드백 페이지에서 확인
- **외출 사유 통계** — 기간별 외출 사유 빈도 관리자 조회
- **DB 자동 백업** — 매일 자정 자동 백업, 최근 7일치 보관
- **그룹 관리** — C.C, IT, C.S 등 소그룹별 관리자 지정, 그룹별 공지 타게팅 가능
- **클라이언트 자동 업데이트** — 서버에 새 EXE 올리면 다음 실행 시 자동 업데이트
- **완전 자동 배포** — push만 하면 서버 배포·클라이언트 빌드·동기화 자동 처리 (self-hosted runner)
- **피드백 / 버그 신고** — 부원이 직접 버그·기능제안·의견 제출, 관리자 페이지에서 확인/삭제 및 코멘트
- **공지사항** — 관리자가 공지 등록 (그룹 타게팅 가능), 부원 로그인 시 팝업으로 표시
...
서버 첫 실행 시 기본 superadmin 계정이 자동 생성됩니다. **최초 로그인 즉시 비밀번호를 변경하세요.** 기본 자격증명은 소스(`server/main.py`)에서 확인할 수 있으며, 운영 환경에서는 반드시 변경해야 합니다.
